...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
	...
	if (!protocolFunc) {
		...
		var processMetadataCallback = function(type, data, time) {
			if (self.stream_ && self.stream_.type === "live" && type === 'ID3') {
				// MD Cast SDK Integration. 
				// onID3Metadata must ne called on any id3 updates from the player host.
				self.stream_.onID3Metadata(data, time);
			}
		};
		if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
			...
			var senderData = info.message.media.customData;
			var contentId = info.message.media.contentId;
			...
			//streamManager.onStreamLoaded = function(stream) {
				...
				//self.stream_ = stream;
				host.url = 'https://cbsios-vh.akamaihd.net/i/temp_hd_gallery_video/CBS_Production_Outlet_VMS/video_robot/CBS_Production_Entertainment/2017/02/19/880378435780/CBS_2_BROKE_GIRLS_617_CONTENT_CIAN_vr_20M_1053680_,1848000,548000,158000,2596000,1248000,298000,3596000,848000,.mp4.csmil/master.m3u8?hdnea=st=1488472083~exp=1488508083~acl=/i/temp_hd_gallery_video/CBS_Production_Outlet_VMS/video_robot/CBS_Production_Entertainment/2017/02/19/880378435780/CBS_2_BROKE_GIRLS_617_CONTENT_CIAN_vr_20M_1053680_,1848000,548000,158000,2596000,1248000,298000,3596000,848000,.mp4.csmil/*~hmac=ed0aea6bdeadee32d39cb22890dd750d76ce5867e4a207b6845441d773c45920&originpath=/cbs/vod/11b87794-b02c-4c44-8a71-50643e92f8e9/hd_manifest.m3u8';
				self.player_ = new cast.player.api.Player(host);
				/*if (self.stream_.type === "vod")*/ self.player_.load(protocolFunc(host));
				// To start playback from the closest point into the live streams, pass Infinity to MPL load call 
			...
			streamManager.adBreaksLoaded = function(adBreaks) {
				console.log("### StreamManager: AdBreaks Loaded! " + JSON.stringify(adBreaks));
			};
			streamManager.shouldProcessAdBreak = function(adBreak) {
				// console.log("### StreamManager: Should Process AdBreak? " + JSON.stringify(adBreak));
				return true;
			};
			streamManager.adBreakStarted = function(adBreak, timeRemaining) {
				console.log("### StreamManager: AdBreak Started! " + JSON.stringify(adBreak) + " timeRemaining - " + timeRemaining);
			};
			streamManager.adBreakEnded = function(adBreak) {
				console.log("### StreamManager: AdBreak Ended! " + JSON.stringify(adBreak));
			};
			streamManager.adEventStarted = function(adEvent) {
				console.log("### StreamManager: Ad Event Started! " + JSON.stringify(adEvent));
			};
			streamManager.onNearestPreviousAdBreak = function(adBreak) {
				var isUnderflow = self.player_ && self.player_.getState()['underflow'];
				if (isUnderflow || Math.floor(self.mediaElement_.currentTime) == adBreak.startTime) return;
				console.log("### StreamManager: NearestPreviousAdBreak after seek @ " + self.mediaElement_.currentTime + " -> " + JSON.stringify(adBreak));
				if (adBreak.isUnwatched) self.mediaElement_.currentTime = adBreak.startTime;
			};
...
sampleplayer.CastPlayer.prototype.onPlaying_ = function() {
	...
	// MD Cast SDK Integration
	// console.log("### Stream: AdBreaks - " + this.stream_.getAdBreaks());
	console.log("### Stream: midRollStartTimes (ad markers) - " + this.stream_.getMidRollStartTimes());
	// console.log("### Stream: nearestPreviousAdBreakForTime @ 1000 (snapback) - " + this.stream_.getNearestPreviousAdBreakForTime("1000").startTime);
	var curTime = this.mediaElement_.currentTime;
	console.log("### Stream: getStreamTimeWithoutAds @ " + curTime + " --- " + this.stream_.getStreamTimeWithoutAds(curTime));
	console.log("### Stream: getStreamTimeWithAds @ " + curTime + " --- " + this.stream_.getStreamTimeWithAds(curTime));
};
/**
 ...
 */
// MD Cast SDK Integration
// INFO: If you receive mediaStatus with customData containing "MDIALOG_CAST_SDK_STATUS" key, 
// please ensure to pass the "MDIALOG_CAST_SDK_STATUS" value AS IS with any other custom data 
